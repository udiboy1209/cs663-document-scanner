from docscanner.stitching import merge_incremental
...
imgs = [cv2.resize(cv2.imread(f,0), (0,0), fx=0.75, fy=0.75) for f in img_files]
imgs_bw = []
for img in imgs:
    ...
    # _,img_bw = cv2.threshold(cv2.resize(img, (0,0), fx=SCALE, fy=SCALE),
    #                        avg, 255, cv2.THRESH_OTSU)

    img_bw = cv2.resize(img, (0,0), fx=SCALE, fy=SCALE)
